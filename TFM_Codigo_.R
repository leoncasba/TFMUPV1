...
data <- read_xlsx("datos_tfm.xlsx") 
dim(data)

#Funcion para detectar datos faltantes por variable
...
write_xlsx(data2, "tfm_imputada.xlsx") #exportar datos imputados Excel para evaluar con PCA outliers

####################################
# PCA - solo validacion de resultados obtenida ASPEN ProMv
####################################

data2 <- read_xlsx("tfm_imputada_ASPEN.xlsx") #datos imputados y eliminando atipicos PCA Aspen
dim(data2)

#PCA1 - Funcion PCA
res.pca = PCA(data2[,-c(1:3)], scale.unit=TRUE, ncp=5, graph=T)

#PCA2 - Funcion ACP
acp<-princomp(data2[,-c(1:3)], scale=TRUE, cor = T)
acp$center
summary(acp)
...
#biplot
fviz_pca_biplot(acp, label="var", labelsize=5, addEllipses=TRUE, ellipse.level=0.95)+theme_minimal() #sinrotar
fviz_pca_biplot(acp, label="var", labelsize=5, addEllipses=TRUE, ellipse.level=0.95, axes=c(1,3))
...
scaled_values <- apply(data2[,c(4:12)], 2, scale, center = TRUE, scale = TRUE)
dim(scaled_values)
#Verificar si estan correctamente escalados
paste("Los datos escalados tienen media", mean(round(apply(scaled_values, 2, mean,na.rm=TRUE))), "y desviacion", mean(apply(scaled_values, 2, sd, na.rm=TRUE)))
...
for (i in 1:10) {
  test <- kmeans(scaled_values, centers = i, nstart = 10)
  # 
...
index_DB6 <- index.DB(k_[, -c(8:9, 12)], k_$k6, d=NULL, centrotypes="centroids", p=2, q=2)
index_DB2$DB
index_DB3$DB
index_DB4$DB
index_DB5$DB
...
kmeans2 <- kmeans(scaled_values, centers=2, nstart = 5)
fviz_cluster(kmeans2, data = scaled_values,
...
fuzzy2 <- cmeans(x=scaled_values, centers = 2, iter.max=100, dist='euclidean', method='cmeans')
fuzzy2$centers
fuzzy2$membership

#Grafico de kmeans+Proyeccion PCA

PCAdata <- prcomp(data2[,c(4:12)], scale=TRUE)

fviz_pca_biplot(PCAdata, label="n", habillage=kmeans2$cluster,
                addEllipses=TRUE, ellipse.level=0.95, title ="K-means + Proyección del PCA")

fviz_pca_biplot(PCAdata, label="n", habillage=fuzzy2$cluster,
                addEllipses=TRUE, ellipse.level=0.95, title ="Fuzzy-Clustering + Proyección del PCA")
...
empresas_k_clusters <- mutate(data2[, c(4:12)], Cluster = clust_customers)
...
# Calcular el tamanio de cada cluster
...
###########################################
#Modelos de clasificacion con todas las variables
##########################################

huella <- read_excel("tfmobjetivo2.xlsx")
huella$Cluster <- factor(huella$Cluster)

#Balanceo de clases para interpretar plsda en Aspen 
...
#Hold out repetido modelos clasificacion 100 iteraciones
...
#DT
rpart.plot(arbol, type = 3, box.palette = c("red", "green"), fallen.leaves = TRUE)

###########################################
#Modelos cribados - variables reducidas
##########################################

huella_cribado <- read_excel("tfmobjetivo2cribado.xlsx")
